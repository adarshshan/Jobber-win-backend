  ...
  async landingPageJobs(req: Request, res: Response) {
    try {
      ...
      res.json({
        success: true,
        data: jobs,
        message: "Landing page jobs fetched successfully.",
      });
    } catch (error) {
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          success: false,
          message: "Internal server error while fetching landing page jobs.",
        });
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while fetching landing page jobs.",
        });
      } else {
  ...
  async getAllJobs(req: Request, res: Response) {
    try {
      ...
      res.json({
        success: true,
        data: result,
        message: "All job details fetched successfully.",
      });
    } catch (error) {
      if (error instanceof DatabaseError) {
  ...
  async getSingleJobDetails(req: Request, res: Response) {
    try {
      ...
      if (error instanceof NotFoundError) {
        ...
        console.error(
          "Database error in getSingleJobDetails controller:",
          error
        );
        res.status(500).json({
          success: false,
        ...
        console.error(
          "Unexpected error in getSingleJobDetails controller:",
          error
        );
        res.status(500).json({
          success: false,
    ...
    try {
      ...
      if (!userId || !jobId) {
        return res.status(400).json({
          success: false,
          message: "User ID or Job ID is not available.",
        });
      }
      ...
      if (error instanceof NotFoundError) {
        res.status(404).json({
          success: false,
          message: "Job or user not found for application.",
        });
      } else if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while applying for job.",
        });
      } else {
        console.error("Unexpected error in applyJOb controller:", error);
        res.status(500).json({
          success: false,
          message: "An unexpected error occurred while applying for job.",
        });
      }
    }
  }
    ...
    try {
      ...
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while fetching applied jobs.",
        });
      } else {
    ...
    try {
      ...
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          success: false,
          message:
            error.message || "An unexpected error occurred while saving job.",
        });
      } else {
        console.error("Unexpected error in saveJobs controller:", error);
        res.status(500).json({
          success: false,
          message: "An unexpected error occurred while saving job.",
        });
      }
    }
  }
  async unSaveJobs(req: Request, res: Response) {
    try {
      ...
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          success: false,
          message: "Internal server error while unsaving job.",
        });
      } else if (error instanceof Error) {
        ...
        res.status(500).json({
          success: false,
          message:
            error.message || "An unexpected error occurred while unsaving job.",
        });
      } else {
        console.error("Unexpected error in unSaveJobs controller:", error);
        res.status(500).json({
          success: false,
          message: "An unexpected error occurred while unsaving job.",
        });
      }
    }
  }
    ...
    try {
      ...
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while fetching saved jobs.",
        });
      } else {
  ...
  async reportJob(req: Request, res: Response) {
    try {
      ...
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          success: false,
          message: "Internal server error during job report.",
        });
      } else if (error instanceof Error) {
        ...
        res.status(500).json({
          success: false,
          message:
            error.message || "An unexpected error occurred during job report.",
        });
      } else {
        console.error("Unexpected error in reportJob controller:", error);
        res.status(500).json({
          success: false,
          message: "An unexpected error occurred during job report.",
        });
      }
    }
  }
  ...
  async getJobsByDate(req: Request, res: Response) {
    try {
      ...
      if (!num) {
        return res
          .status(400)
          .json({ success: false, message: "Number of days is required." });
      }
      const result = await this.jobService.getJobsByDate(num);
      res.json({
        success: true,
        data: result,
        message: "Jobs by date fetched successfully.",
      });
    } catch (error) {
      if (error instanceof DatabaseError) {
        ...
        res.status(500).json({
          success: false,
          message: "Internal server error while fetching jobs by date.",
        });
      } else if (error instanceof Error) {
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while fetching jobs by date.",
        });
      } else {
        console.error("Unexpected error in getJobsByDate controller:", error);
        res.status(500).json({
          success: false,
          message: "An unexpected error occurred while fetching jobs by date.",
        });
      }
    }
  }
  async getJobsByExperience(req: Request, res: Response) {
    try {
      ...
      res.json({
        success: true,
        data: result,
        message: "Jobs by experience fetched successfully.",
      });
    } catch (error) {
      if (error instanceof DatabaseError) {
        console.error(
          "Database error in getJobsByExperience controller:",
          error
        );
        res.status(500).json({
          success: false,
          message: "Internal server error while fetching jobs by experience.",
        });
      } else if (error instanceof Error) {
        ...
        res.status(500).json({
          ...
          message:
            error.message ||
            "An unexpected error occurred while fetching jobs by experience.",
        });
      } else {
        console.error(
          "Unexpected error in getJobsByExperience controller:",
          error
        );
        res.status(500).json({
          success: false,
