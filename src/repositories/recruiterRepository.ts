...
class RecruiterRepository implements IRecruiterRepository {
    ...
    async getAllJobs(recruiterId: string): Promise<any[]> {
        try {
            ...
            throw new DatabaseError(`Failed to retrieve jobs for recruiter ID ${recruiterId}.`, error as Error);
        }
    }
    async postNewJob(data: JobInterface, userId: string): Promise<any> {
        try {
            ...
            throw new DatabaseError(`Failed to post new job for recruiter ID ${userId}.`, error as Error);
        }
    }
    async editJobs(data: JobInterface, jobId: string): Promise<any> {
        try {
            ...
            throw new DatabaseError(`Failed to edit job with ID ${jobId}.`, error as Error);
        }
    }
    async deleteJob(jobId: string): Promise<void> { // Assuming jobId is passed for deletion
        try {
            ...
            throw new DatabaseError(`Failed to delete job with ID ${jobId}.`, error as Error);
        }
    }
    async changeStatus(status: string, applicationId: string): Promise<any> {
        try {
            ...
            if (!application) {
                throw new NotFoundError(`Job application with ID ${applicationId} not found.`);
            }
            ...
            throw new DatabaseError(`Failed to change status for application ID ${applicationId}.`, error as Error);
        }
    }
}
